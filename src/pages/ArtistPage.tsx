    ...
    <section className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex items-start justify-between gap-3">
          <div>
            {artist.profileImage ? <img src={artist.profileImage} alt={artist.name} className="mb-3 h-40 w-40 rounded-xl object-cover" /> : null}
            <h1 className="text-3xl font-black text-slate-900 dark:text-white">{artist.name}</h1>
            <p className="mt-3 text-sm leading-relaxed text-slate-700 dark:text-slate-300">{artist.biography}</p>
          </div>
          <button onClick={toggleFollow} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white">
